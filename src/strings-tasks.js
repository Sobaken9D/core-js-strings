...
function isPalindrome(str) {
  const newStr = str.toLowerCase().replace(/[^\w\d]|_/g, '');
...
function findLongestWord(sentence) {
  ...
  const res = arr.find((el) => el.length === Math.max(...arrLen));
  return res;
}
...
function encodeToRot13(str) {
  ...
  return arr
    .map((el) => {
      ...
      if (code >= 65 && code <= 90) {
        code = code + 13 > 90 ? 65 + (code + 13 - 91) : code + 13;
      }
      if (code >= 97 && code <= 122) {
        code = code + 13 > 122 ? 97 + (code + 13 - 123) : code + 13;
      }
